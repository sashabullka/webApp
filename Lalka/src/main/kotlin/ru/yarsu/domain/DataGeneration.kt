...
fun generation(){
    ...
    val peopleID: MutableSet<String> = mutableSetOf()
    while(peopleID.size < 1000){
        peopleID.add(id())
    }
    val peopleList: List<String> = peopleID.toList()

    val proectID: MutableSet<String> = mutableSetOf()
    while(proectID.size < 1000){
        proectID.add(id())
    }
    val proectList: List<String> = peopleID.toList()
    ...
    for (i in 0..9) {
        ...
        for (j in 0..99) {
            array += Golos(People(peopleList[ind],cities(i)),dateGenerationPeople(projectDate[i].toString().split("-")[0].toInt()), nickNamee())
            ind += 1
        }
        people += array
    }
    ...
    for(i in 0..9) {
        proect.add(Proect(cities(i), street(i), coordinates(), description(i), people[i].toMutableList(), projectDate[i], proectList[i]
        ))
    }
