...
class GolosGetDeleteHandler(val renderer: ContextAwareViewRender, val listProects: MutableList<Proect>, val getProectByName: GetProectByName, val storage: ProectsStorage, val permissionLens: RequestContextLens<Permisson?>) : HttpHandler {
    override fun invoke(request: Request): Response {
        ...
        val viewModel = GolosDeleteVM(golos)
        val permissions = permissionLens(request)
        return if (permissions != null && permissions.canDeleteGolos) {
