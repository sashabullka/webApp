...
import ru.yarsu.web.models.ProectDeleteVM
import ru.yarsu.web.templates.ContextAwareViewRender

class ProectGetDeleteHandler(val renderer: ContextAwareViewRender, val listProects: MutableList<Proect>, val getProectByName: GetProectByName, val storage: ProectsStorage, val permissionLens: RequestContextLens<Permisson?>) : HttpHandler {
    ...
    override fun invoke(request: Request): Response {
        ...
        val viewModel = ProectDeleteVM(proect)
        val permissions = permissionLens(request)
        return if (permissions != null && permissions.canDeleteProect) {
