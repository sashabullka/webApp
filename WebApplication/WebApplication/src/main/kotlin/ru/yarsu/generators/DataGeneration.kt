...
fun dateGenerationProject(): LocalDateTime? { val minDay = LocalDate.of(2013, 12, 31).toEpochDay()
...
fun registerUser(username: String, password: String, dateTime: LocalDateTime, role: String, salts: MutableMap<String,String>, users: MutableList<User>) {
...
fun generation(){
    ...
    val peopleID: MutableSet<Int> = mutableSetOf()
    while(peopleID.size < 1000){
        peopleID.add(id())
    }
    val peopleList: List<Int> = peopleID.toList()

    val proectID: MutableSet<Int> = mutableSetOf()
    while(proectID.size < 1000){
        proectID.add(id())
    }
    val proectList: List<Int> = proectID.toList()

    var ind = 0
    var people = arrayOf<Array<Golos>>()
    val users = mutableListOf<User>()
    val salt =  mutableMapOf<String, String>()
    for (i in 0..9) {
        var array = arrayOf<Golos>()
        for (j in 0..99) {
            array += Golos(People(peopleList[ind],cities(i), "Неизвестный пользователь"),dateGenerationPeople(projectDate[i].toString().split("-")[0].toInt()), nickNamee())
            ind += 1
        }
        people += array
    }
    registerUser("admin", "admin", LocalDateTime.now(), "admin", salt, users)
    registerUser("cityAdminUser", "cityAdminUser", LocalDateTime.now(), "cityAdmin", salt, users)
    registerUser("citizenUser", "citizenUser", LocalDateTime.now(), "citizen", salt, users)
    ...
    for(i in 0..9) {
        proect.add(
            Proect(cities(i), street(i), coordinates(), description(i), people[i].toMutableList(), projectDate[i], proectList[i], "admin"
        )
        )
    }
